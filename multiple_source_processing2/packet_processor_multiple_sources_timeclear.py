...
class packet_processor:
    ...
    def packetProcessor (self):
        ...
        gravPackets = []
        accPackets = []
        packetsCombinedX = 0.0
        packetsCombinedY = 0.0
        packetsCombinedZ = 0.0

        for j in range(len(accPacketsTogether)):
            try:
                if accPacketsTogether[j][0] == accPacketsTogether[j+1][0]:
                    packetsCombinedX = ( float(accPacketsTogether[j][1]) + float(accPacketsTogether[j+1][1]) )/2 
                    packetsCombinedY = ( float(accPacketsTogether[j][2]) + float(accPacketsTogether[j+1][2]) )/2 
                    packetsCombinedZ = ( float(accPacketsTogether[j][3]) + float(accPacketsTogether[j+1][3]) )/2 
                    accPacketsTogether[j] = [accPacketsTogether[j][0], packetsCombinedX, packetsCombinedY, packetsCombinedZ ]
                    accPacketsTogether.pop(j+1)
            except IndexError:
                pass
        ...
        for j in range(len(gravPacketsTogether)):
            try:
                if gravPacketsTogether[j][0] == gravPacketsTogether[j+1][0]:
                    packetsCombinedX = (float( gravPacketsTogether[j][1]) + float(gravPacketsTogether[j+1][1]) ) / 2 
                    packetsCombinedY = (float( gravPacketsTogether[j][2]) + float(gravPacketsTogether[j+1][2]) ) / 2 
                    packetsCombinedZ = (float( gravPacketsTogether[j][3]) + float(gravPacketsTogether[j+1][3]) ) / 2 
                    gravPacketsTogether[j] = [gravPacketsTogether[j][0], packetsCombinedX, packetsCombinedY, packetsCombinedZ ]
                    gravPacketsTogether.pop(j+1)
            except IndexError:
                pass
                #source for ignoring the index out of range error, so that you can remove elements from this list. It's not a concern
                # https://stackoverflow.com/a/26853522
        ...
        """
        WHAT IS HAPPENING WITH THE CASE WHEN WE HAVE THE SAME TIMESTAMP FOR DIFFERENT ACC/GRAV PACKETS. THIS IS A PROBLEM. WHAT ABOUT n MANY PACKETS? 
        gravPacketsNoMultiples = []
        accPacketsNoMultiples = []

        
        for i in range(len(gravPackets):
            
            for j in range(len(gravPackets)):

                while (i < j) and (gravPackets[i][0] - gravPackets[j][0] == 0 ):

                        gravPacketsNoMultiples.append( ( gravPackets[i][1] + gravPackets[j][1] )/2 )
